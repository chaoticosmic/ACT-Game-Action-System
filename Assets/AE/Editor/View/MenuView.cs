    ...
    public class MenuView : IView
    {
        ...
        private void PlayMode()
        {
            ...
            if (EditorGUI.EndChangeCheck() || !isInit)
            {
                ...
                var config = ActionMachineHelper.loadedConfig[selectConfigName];
                win.config = config;
            }
        ...
        private void PasteConfig()
        {
            ...
            ActionInfoContainer config;

            try
            {
                string data = EditorGUIUtility.systemCopyBuffer;
                config = XMLib.DataUtility.FromJson<ActionInfoContainer>(data);
                if (config == null)
                {
                    throw new Exception();
                }
        ...
        private void CopyConfig()
        {
            ...
            string data = XMLib.DataUtility.ToJson(win.config);
        ...
        private void CreateNew()
        {
            ...
            string filePath = EditorUtilityEx.ValidFilePath(Path.Combine(dir, "MachineConfig.json"));
            string data = XMLib.DataUtility.ToJson(new ActionInfoContainer());
        ...
        private void CopyToNew(ActionInfoContainer config)
        {
            string dir = EditorUtilityEx.GetSelectDirectory();
            string filePath = EditorUtilityEx.ValidFilePath(Path.Combine(dir, "MachineConfig.json"));
            string data = XMLib.DataUtility.ToJson(config);
            File.WriteAllText(filePath, data);
            AssetDatabase.Refresh();
        ...
        private void SaveConfigToSelect(ActionInfoContainer config)
        {
            string filePath = EditorUtilityEx.GetSelectFile(".json");
